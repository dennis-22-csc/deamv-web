...
interface AutoSpeakProps {
	...
	onStart?: () => void;
	onEnd?: () => void;
	onError?: (error: string) => void; // Include onError prop here
}
...
export const AutoSpeak: React.FC<AutoSpeakProps> = ({
	...
	useEffect(() => {
		...
		const timer = setTimeout(() => {
			...
			// FIX: Wrap optional simple props to match SpeechSynthesisUtterance event handler signature.
			// Handlers must accept an event object (which we ignore) or be null.
			utterance.onstart = onStart ? () => onStart() : null;
			utterance.onend = onEnd ? () => onEnd() : null;
			
			// FIX: Correctly call the onError prop with the event.error string, or null
			utterance.onerror = onError ? (event) => onError(`Speech error: ${event.error}`) : null;
	...
	}, [text, enabled, delay, onStart, onEnd, onError]);
