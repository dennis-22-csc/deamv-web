...
// Map ConfirmationDialog's custom variants to base Dialog variants
type ConfirmationVariant = 'default' | 'danger' | 'warning' | 'info';
const mapVariant = (variant: ConfirmationVariant): DialogProps['variant'] => {
    switch (variant) {
        case 'danger':
            return 'error';
        case 'info':
            return 'alert'; // Using 'alert' for general info/non-error blue variant
        case 'warning':
            return 'warning';
        case 'default':
        default:
...
interface ConfirmationDialogProps {
	...
	confirmText?: string;
	cancelText?: string;
	variant?: ConfirmationVariant; // Use the custom type
...
const ConfirmationDialog: React.FC<ConfirmationDialogProps> = ({
	...
	return (
		<Dialog
			...
			onClose={onClose}
			title={title}
			// FIX 2A: Map custom variant to base Dialog variant
			variant={mapVariant(variant)}
			confirmText={confirmText}
			cancelText={cancelText}
			onConfirm={handleConfirm}
			// FIX 2B: Remove onCancel - use onClose instead
			loading={loading}
...
const CustomConfirmDialog: React.FC<CustomConfirmDialogProps> = ({
    ...
    // FIX 2B: Remove onCancel and map variant
	return (
		<Dialog
			{...props}
			variant={mapVariant(props.variant || 'default')}
			confirmText={props.confirmText || 'Confirm'}
			cancelText={props.cancelText || 'Cancel'}
			onConfirm={props.onConfirm}
			onClose={props.onClose} // Use onClose instead of onCancel
