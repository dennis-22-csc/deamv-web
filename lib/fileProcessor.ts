// File processing logic for questions files (replaces Android FileProcessor)
...
class FileProcessor {
  ...
  /**
   * FIXED: Parse CSV content using PapaParse for reliable quote and newline handling.
   */
  private parseCsvContent(content: string, fileName: string): ParsedChallenge[] {
    ...
    try {
      ...
      // Use PapaParse to handle the complex CSV structure (quotes, multi-line fields)
      ...
      data.forEach((row, index) => {
        ...
        const instruction = row[instructionKey]?.trim() || '';
        const solution = row[solutionKey]?.trim() || '';
