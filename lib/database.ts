...
class PracticeDatabase {
  ...
  async getDataScienceChallengesByCategory(category: string): Promise<DataScienceChallenge[]> {
    try {
      const challenges = await this.getDataScienceChallenges();
  ...
  async updateChallengeProgress(
    ...
    try {
      ...
      categoryStats.completed = progress.completedChallenges.filter(id => {
        // Using a direct find/filter might be slow. Optimization note.
        // For simplicity, keeping existing logic for now.
        return this.getChallengeById(id).then(c => c?.category === category);
      }).length;
      ...
      const categoryScores = Object.entries(progress.scores)
        .filter(([id]) => {
          // Optimization note: Pre-fetch all challenges for performance
          return this.getChallengeById(id).then(c => c?.category === category);
        })
        .map(([, score]) => score);
